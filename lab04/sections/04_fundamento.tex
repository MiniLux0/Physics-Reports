...
\end{equation}
donde $g$ es la aceleración de la gravedad. De aquí se obtiene la masa efectiva equivalente:
\begin{equation}
    M_{\text{ef}} = \sum_{j} m_j \,\frac{n_j}{10}
    \label{eq:masa_ef}
\end{equation}
y la fuerza de tensión superficial asociada:
\begin{equation}
    F_{TS} = M_{\text{ef}}\, g.
    \label{eq:fuerza_ts}
\end{equation}
...
\end{equation}
donde $T$ es la tensión en el hilo y $P = mg$ el peso del tubo inferior. Cabe destacar que **el factor 2** que multiplica a $\gamma$ en ambos balances obedece a que la película jabonosa posee dos superficies libres (frontal y posterior), ejerciendo tensión a lo largo de cada una de ellas.
